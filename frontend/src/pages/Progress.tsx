...
import { ArrowLeft, Target, Clock, Zap, CheckCircle, BarChart3 } from "lucide-react"
...
export default function Progress() {
  ...
  const loadProgressData = async () => {
    try {
      ...
      try {
        const response = await axios.get("https://tsinjool-backend.onrender.com/api/progress/", {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
          ...
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Suivez votre évolution et vos accomplissements</p>
              </div>
            </div>

            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm text-gray-600">Niveau {stats.current_level}</p>
                <p className="text-lg font-bold text-purple-600">{stats.total_points} pts</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{stats.total_exercises_completed}</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {Math.floor(stats.total_time_spent / 60)}h {stats.total_time_spent % 60}min
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{stats.current_streak} jours</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{Math.round(stats.overall_progress)}%</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Progression du niveau</h3>

            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-600">Niveau {stats.current_level}</span>
              <span className="text-sm font-medium text-gray-600">Niveau {stats.current_level + 1}</span>
            </div>
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-600 mb-1">{stats.total_points} points</p>
              <p className="text-sm text-gray-600">{100 - (stats.total_points % 100)} points pour le niveau suivant</p>
            </div>
          </div>

          {/* Série quotidienne */}
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Série quotidienne</h3>

            <div className="text-center mb-4">
              <div className="text-4xl mb-2">🔥</div>
              <p className="text-3xl font-bold text-orange-600 mb-1">{stats.current_streak}</p>
              <p className="text-sm text-gray-600">jours consécutifs</p>
            </div>

            <div className="bg-orange-50 rounded-lg p-4">
              <p className="text-center text-orange-800 font-medium">{getStreakMessage(stats.current_streak)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm mb-8">
          <h3 className="text-lg font-semibold text-gray-900 mb-6">Activité de la semaine</h3>

          <div className="grid grid-cols-7 gap-4">
            {weeklyData.map((day, index) => (
              <div key={index} className="text-center">
                <div className="text-sm font-medium text-gray-600 mb-2">{day.day}</div>
                <div className="w-full bg-gray-200 rounded-lg flex flex-col justify-end" style={{ height: "100px" }}>
                  <div
                    className="bg-gradient-to-t from-purple-500 to-blue-600 rounded-lg transition-all duration-500"
                    style={{ height: `${Math.max((day.exercises / 5) * 100, 10)}%` }}
                  />
                </div>
                <div className="text-xs text-gray-500 mt-2">{day.exercises} ex.</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Accomplissements */}
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h3 className="text-lg font-semibold text-gray-900 mb-6">Accomplissements</h3>
            ...
            {achievements.map((achievement) => (
              ...
              >
                <div className="text-center">
                  ...
                  <h4 className={`font-semibold mb-1 ${achievement.unlocked ? "text-green-800" : "text-gray-600"}`}>
                    {achievement.title}
                  </h4>
